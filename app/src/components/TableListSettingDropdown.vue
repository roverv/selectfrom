<template>
  <div id="dropdown" class="relative inline-block">
    <a @click="isOpen = true" class="settings-toggle-btn">
      <svg viewBox="0 0 24 24" class="text-light-200 ml-1" :class="{ 'text-dark-600' : isOpen}">
        <path class="primary"
    ...
    >
      <div v-if="isOpen"
        ...
        <div class="w-auto bg-dark-500 text-gray-200 rounded-lg shadow-lg">
          <div class="w-64 py-5">
            <a class="block px-6 py-1 leading-tight" @click="goToCreateTable">Create new table</a>
            <hr class="border-dark-400 my-2 mx-5">
            <a class="block px-6 py-1 leading-tight" @click="$emit('toggleTablesWithoutRows'), isOpen=false">
              <span v-if="only_show_tables_with_rows">Show</span>
              <span v-else>Hide</span>
              tables without data
            </a>
            <hr class="border-dark-400 my-2 mx-5">
            <a class="block px-6 py-1 leading-tight" @click="$emit('toggleTableList'),  isOpen=false">
